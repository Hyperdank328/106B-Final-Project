...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    try:
        ...
        trans = tfBuffer.lookup_transform('base', 'stp_022312TP99620_tip_1', rospy.Time(0), rospy.Duration(10.0))
    except Exception as e:
        print(e)
    ...
    target_pos[2] += 0.4
    if task == 'line':
        trajectory = LinearTrajectory(current_position, target_pos, 9)
    elif task == 'circle':
        trajectory = CircularTrajectory(target_pos, 0.1, 15)
    elif task == 'polygon':
...
def get_controller(controller_name, limb, kin):
    ...
    if controller_name == 'workspace':
        Kp = 0.7 * np.array([7, 7 ,2, 0, 0, 0])
        Kv = np.array([0,0,0,0,0,0]) # we don't use this value
        controller = WorkspaceVelocityController(limb, kin, Kp, Kv)
    elif controller_name == 'jointspace':
        Kp = 0.2 * np.array([0.7, 2, 1.7, 2, 2, 2, 3])
        Kv = 0.01 * np.array([2.5, 1, 2, 0.7, 0.8, 0.8, 0.8])
        controller = PDJointVelocityController(limb, kin, Kp, Kv)
    elif controller_name == 'torque':
        Kp = 35.0 * np.array([4,1,3,1.5,2,1,2])
        Kv = 6.0 * np.array([1,1,1,1,1,1,1])
...
def main():
    ...
    parser.add_argument('-controller_name', '-c', type=str, default='moveit', 
        help='Options: moveit, osawyer_kinematicspen_loop, jointspace, workspace, or torque.  Default: moveit'
    )
    parser.add_argument('-rate', type=int, default=200, help="""
        This specifies how many ms between loops.  It is important to use a rate
        and not a regular while sawyer_kinematics faster.  Default: 200"""
    )
