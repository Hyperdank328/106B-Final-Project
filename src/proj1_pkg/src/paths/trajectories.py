...
class LinearTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            pos = ...
            dir = self.goal_position - self.start_position
            # normalize direction vector
            dir_norm = dir / np.linalg.norm(dir)
            pos = self.acceleration * (time ** 2) / 2.0 + self.start_position
        else:
            # TODO: Calculate the position of the end effector at time t, 
            ...
            pos = ...
            time = time - self.total_time / 2.0
            velocity = self.acceleration * self.total_time / 2.0
            dir = self.goal_position - self.start_position
            dir_norm = dir / np.linalg.norm(dir)
            pos = velocity * time -self.acceleration * (time ** 2) / 2.0 + self.start_position + dir / 2.0
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            linear_vel = (self.acceleration * time) * unit_dir
        else:
            ...
            time = time - self.total_time / 2.0
            linear_vel = (self.v_max - self.acceleration * time) * unit_dir
        Vs = np.hstack((linear_vel, np.zeros(3)))

        position = self.target_pose(time)[:3]
        quaternion = self.target_pose(time)[3:]
        g = utils.get_g_matrix(position, quaternion)
        adj_g = np.linalg.inv(utils.adj(g))
...
class CircularTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            # For the first half of the trajectory, maintain a constant acceleration
            theta = theta_i + self.angular_acceleration * (time ** 2) / 2.0
        else:
            # TODO: Calculate the ANGLE of the end effector at time t, 
            # For the second half of the trajectory, maintain a constant acceleration
            ...
            time = time - self.total_time / 2.0
            theta = np.pi + self.angular_v_max * time - self.angular_acceleration * (time ** 2) / 2.0
    ...
    def target_velocity(self, time):
        ...
        Parameters        print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        time : float
...
if __name__ == '__main__':
    ...
    # path = CircularTrajectory(np.array([0.2, 0.4, 0.6]), .3, 20)
